...
for (const slug of slugs) {
  ...
  try {
    ...
    // The cache file's mtime is when this repo was measured. Rows are not all
    // the same age: a rerun only re-measures what was deleted.
    measuredAt = (await stat(file)).mtime;
  } catch { continue; }
  rows.push({ slug, head: r.head, reference: r.reference, measuredAt, ...summarise(r) });
}
...
const stamps = rows.map((r) => r.measuredAt).filter(Boolean).sort((a, b) => a - b);
L.push(`Generated by \`node panel/run.js\` from the repo list in [\`panel/repos.txt\`](panel/repos.txt),`);
L.push(`with \`holdover\` ${version}, measured ${day(stamps[0]) === day(stamps.at(-1))
...
L.push('');
L.push('| repo | tip | tip arrived | measured |');
L.push('| --- | --- | --- | --- |');
for (const r of rows) {
  if (!r.head) continue;
  L.push(`| \`${r.slug}\` | [\`${r.head.slice(0, 10)}\`](https://github.com/${r.slug}/commit/${r.head}) `
    + `| ${r.reference ? day(r.reference * 1000) : '—'} | ${day(r.measuredAt)} |`);
}
L.push('');
