...
| --- | --- |
| +32.7 pp | nothing — first working version |
...
> under [Prior art](#prior-art) with what they do differently.
...
| state | meaning |
| --- | --- |
| `kept` | the line is still there, unchanged apart from whitespace. Either `git blame` still credits it to the commit that wrote it — following content moved between files, ignoring the revisions the repo asks blame to ignore — or the diff from that commit to HEAD does not touch it at all |
...
attribution. See [the decomposition](#where-the-disagreeing-numbers-come-from).
...
27 of 54 agent commits keep *zero* lines — small fixes, entirely rewritten — while
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `openai/codex` | 1,185,249 | 56.3% | 53.0% | +3.2 pp | +20.8 pp | 34% vs 28% | both estimators agree, composition close — but see the horizon note below |
| `aaif-goose/goose` | 66,090 | 55.3% | 44.6% | **+10.7 pp** | +20.5 pp | 58% vs 48% | both estimators agree, composition close — the cleanest result here |
| `getzep/graphiti` | 2,786 | 76.4% | 81.2% | -4.8 pp | -98.2 pp | 67% vs 26% | estimators 93 points apart, 5 commits are 80% of the cohort — no conclusion |
| `OpenHands/OpenHands` | — | — | — | — | — | — | `unmeasurable`: the tree was replaced wholesale, so both classes retain nothing |

Note the *level*, not just the gap: 56.3%, 55.3%, 76.4%. A headline "AI code survives
...
| | retroactive? | repos you don't own? | needs installing first? | human baseline? | three states? | excludes mixed squashes? |
| --- | --- | --- | --- | --- | --- | --- |
| **holdover** | yes | yes | no | yes | yes | yes |
| [`git-ai`](https://github.com/git-ai-project/git-ai) | no | no | yes — agents must checkpoint as they write | no | n/a (measures provenance, not survival) | n/a |
| `survival.py` in [`Everyone-Needs-A-Copilot/claude-copilot`](https://github.com/Everyone-Needs-A-Copilot/claude-copilot) | yes | yes | no | no | no | no |
| [`keeprate`](https://www.npmjs.com/package/keeprate) | no | no | yes — needs Claude Code session logs | no | no | n/a |
| [`stillthere`](https://www.npmjs.com/package/stillthere) | yes | yes | no | yes | no | no |
| [`git-of-theseus`](https://github.com/erikbern/git-of-theseus) | yes | yes | no | n/a | no | n/a |

The last column is the one that matters most and it is the newest. Every
...
`git-ai` states its position plainly in its own README: "Git AI does not use AI or
heuristics to 'detect' AI code — the Agents report exactly which lines they wrote."
That gives it ground-truth attribution and means it can never measure history that
predates its install, or a repo you merely cloned, because the attribution lives in
git notes that are not pushed by default.
...
full rewrite, has no arrival dating or horizon, credits the whole of a squashed PR
to the agent, and classifies as human every commit its agent patterns did not
match — which puts Cursor, Windsurf, Replit, Jules, Aider's author-rename mode and
every un-attributed agent commit into the baseline it compares against. Its
`Co-authored-by:\s*openai-codex` pattern does not match the trailer Codex actually
writes (`Co-authored-by: Codex <noreply@openai.com>`), and its
`/🤖 generated with/i` pattern credits any tool's emoji footer to Claude, because
first match wins.
...
figure, and its own published number on that exact two-week definition is 3.1% in
2020 rising to 5.7% in 2024. GitClear also has no per-line AI attribution at all —
it compares time periods.
...
2. **Date.** Arrival on the default branch. A commit on the first-parent trunk
   ...
   trunk commit having it as an ancestor arrived — that is, when the merge that
